...
export function generateTrackServer(seed: number): Track {
...
function createMazeSegmentServer(prev: SegmentEnd, width: number, rng: () => number): TrackSegment {
...
function createServerSegment(type: SegmentType, prev: SegmentEnd, width: number, rng: () => number): TrackSegment {
  const hw = width / 2;
  ...
  switch (type) {
    ...
    case 'zigzag': {
      ...
      const leftPts: TrackPoint[] = [{ x: lx, y: ly }];
      const rightPts: TrackPoint[] = [{ x: rx, y: ry }];
      const segH = sh / zigs;
    ...
    case 'split': {
      ...
      const leftRaw: TrackPoint[] = fracs.map((f, i) => {
        ...
        if (i === fracs.length - 1) return { x: x - hw + drift, y: y + sh };
...
export function createServerRace(lobby: Lobby, track: Track, gravityScale: number = 0.0004): ServerRace {
  ...
  const marbleConfigs: MarbleConfig[] = lobby.players.map(p => ({
  ...
  let status: 'countdown' | 'racing' | 'finished' = 'countdown';
  ...
  const stuckTracking = new Map<string, { x: number; y: number; time: number; stuckCount: number }>();
  ...
  function tick() {
    if (status === 'countdown') {
      ...
      if (countdown <= 0) { status = 'racing'; countdown = 0; }
      return;
    }
    ...
    if (status === 'racing') {
      ...
      if (finishedMarbles.size + disqualifiedMarbles.size >= marbleConfigs.length) { status = 'finished'; }
    }
  }

  function getState(): RaceState {
    const marbleStates: MarbleState[] = [];
    for (const [id, body] of marbleBodies) {
...
function createWall(x1: number, y1: number, x2: number, y2: number): Matter.Body {
  const cx = (x1 + x2) / 2;
  const cy = (y1 + y2) / 2;
  const dx = x2 - x1;
  const dy = y2 - y1;
