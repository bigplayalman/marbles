...
import { TICK_RATE, SYNC_RATE, } from '../../shared/types.js';
import { createLobby, joinLobby, leaveLobby, addBot, removeBot, getLobby, getPlayerLobby, setLobbyState, setTrackSeed, } from './lobby.js';
import { createServerRace, generateTrackServer } from './race.js';
const PORT = parseInt(process.env.PORT || '3001');
const HOST = process.env.HOST || '0.0.0.0';
const fastify = Fastify({ logger: true });
// Track connected clients
const clients = new Map(); // playerId -> WebSocket
const activeRaces = new Map(); // lobbyCode -> race
...
function broadcastToLobby(lobbyCode, msg, excludeId) {
...
function handleMessage(playerId, socket, msg) {
    switch (msg.type) {
        case 'create_lobby': {
        ...
        case 'start_race': {
            ...
            const marbleConfigs = playerLobby3.players.map(p => ({
                id: p.id,
                name: p.marbleName,
                color: p.marbleColor,
                isBot: p.isBot,
            ...
            const interval = setInterval(() => {
                ...
                if (tickCount % ticksPerSync === 0) {
                    ...
                    const stateMsg = { type: 'race_state', state: raceState };
